...
class JinaProvider(EmbeddingProvider):
    def __init__(self, base_url: str, api_key: str | None = None, model: str = "jina-embeddings-v3"):
    ...
    def embed_texts(self, texts: list[str], *, task: str = "retrieval.passage", dim: int = 1024) -> list[list[float]]:
...
class ProviderFactory:
    ...
    def create(name: str, **kw) -> EmbeddingProvider:
        if name == "jina":   return JinaProvider(**kw)
        if name == "ollama": return OllamaProvider(**kw)
